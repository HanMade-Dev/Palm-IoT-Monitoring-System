...
/***** WEB SERVER CONFIG *****/
const char* serverURL = "http://your-domain.com/api/receive_data.php";  // Change this to your domain
...
void sendDataToWebServer() {
  ...
  // Create JSON payload dengan semua status
  ...
  // FIXED: set temperature dengan tipe yang konsisten
...
void readSensorsAndSendData() {
  ...
  // FIXED: tipe aman untuk serial JSON juga
  if (temperature == DEVICE_DISCONNECTED_C) {
    serialDoc["temperature"] = "DEVICE_DISCONNECTED_C";
  } else {
    serialDoc["temperature"] = temperature; // float
  }
